...
def test_device_create_valid():
    d = DeviceCreate(device_id="nvr-01", name="Test NVR", vendor="hikvision",
        host="192.168.1.100", web_port=80, sdk_port=None, username="admin", password="secret")
    assert d.device_id == "nvr-01"
    assert d.web_port == 80
...
def test_device_create_default_port():
    ...
    assert d.web_port is None
    assert d.sdk_port is None
...
def test_device_out_with_health():
    ...
    d = DeviceOut(device_id="nvr-01", name="Test", vendor="hikvision",
        host="10.0.0.1", web_port=80, sdk_port=None, transport_mode="isapi", is_active=True, last_health=h)
    assert d.last_health.camera_count == 4

def test_device_out_without_health():
    d = DeviceOut(device_id="nvr-01", name="Test", vendor="hikvision",
        host="10.0.0.1", web_port=80, sdk_port=None, transport_mode="isapi", is_active=True, last_health=None)
